...
export function useDownload(
  ...
  const rowsFor = (scope: ExportScope): ExportRow[] =>
    (scope === 'page' ? pageRows.value : filteredRows.value).map((row) => ({
      key: row.key,
      values: row,
    }))
  const run = async (format: ExportFormat, scope: ExportScope) => {
    ...
    const content =
      format === 'csv'
        ? serializeCsv(rows, languages.value)
        : format === 'tsv'
          ? serializeTsv(rows, languages.value)
          : format === 'json'
            ? serializeJson(rows, languages.value)
            : serializeXml(rows, languages.value)
    return downloadBlob(new Blob([content], { type: mimeTypes[format] }), filename)
  }
  return {
    downloadTsv: () => run('tsv', 'page'),
    downloadCsv: () => run('csv', 'page'),
    downloadJson: () => run('json', 'page'),
    downloadXml: () => run('xml', 'page'),
    downloadXlsx: () => run('xlsx', 'page'),
    downloadAllTsv: () => run('tsv', 'filtered'),
    downloadAllCsv: () => run('csv', 'filtered'),
    downloadAllJson: () => run('json', 'filtered'),
    downloadAllXml: () => run('xml', 'filtered'),
    downloadAllXlsx: () => run('xlsx', 'filtered'),
  }
}
