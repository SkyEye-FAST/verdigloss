...
describe('phase-two data boundaries', () => {
  ...
  it('builds predictable capped search indexes', () => {
    const index = new SearchIndex('en_us', {
      'item.apple': 'Apple',
      'block.apple': 'Apple block',
      stone: 'Stone',
    })
    expect(index.search('key', 'apple')).toHaveLength(2)
    expect(index.search('source', 'APPLE', 1)).toEqual([
      { key: 'item.apple', matchedValue: '', mode: 'source', language: 'en_us' },
    ])
  ...
  it('filters only selected loaded columns and clamps pagination without page zero', () => {
    const dictionaries = {
      ...
      zh_cn: { apple: '苹果', stone: '石头' },
    }
    expect(filterTranslationKeys(['apple', 'stone'], dictionaries, ['zh_cn'], 'stone')).toEqual([
      'stone',
    ])
    expect(filterTranslationKeys(['apple', 'stone'], dictionaries, ['zh_cn'], '苹果')).toEqual([
      'apple',
    ])
  ...
  it('only removes a trailing parenthetical annotation from color labels', () => {
    expect(splitTrailingAnnotation('두 단어 이름 (漢字)')).toEqual({
      label: '두 단어 이름',
      annotation: '(漢字)',
    })
    expect(splitTrailingAnnotation('plain (parenthesis) text')).toEqual({
      label: 'plain (parenthesis) text',
      annotation: '',
    })
  })

  it('gives every route a fallback title and updates the document title for locale changes', async () => {
    expect(router.getRoutes().every((route) => typeof route.meta.fallbackTitle === 'string')).toBe(
      true,
    )
