...
const colors = {
  ...
  gray: ['#474F52', '#808080'],
  green: ['#5E7C16', '#00FF00'],
  light_blue: ['#3AB3DA', '#9AC0CD'],
  light_gray: ['#9D9D97', '#D3D3D3'],
  lime: ['#80C71F', '#BFFF00'],
  magenta: ['#C74EBD', '#FF00FF'],
  orange: ['#F9801D', '#FF681F'],
  pink: ['#F38BAA', '#FF69B4'],
  purple: ['#8932B8', '#A020F0'],
  red: ['#B02E26', '#FF0000'],
  white: ['#F9FFFE', '#FFFFFF'],
  yellow: ['#FED83D', '#FFFF00'],
} as const
...
export function splitTrailingAnnotation(value: string) {
  ...
  return match
    ? { label: match[1]!, annotation: `(${match[2]})` }
...
export const colorDataset = {
  ...
  colors: (Object.keys(colors) as ColorKey[]).map((key) => {
    ...
    return {
      key,
      hex: colors[key][0],
      textHex: colors[key][1],
      icon: dyeIcons[key],
      iconNew: dyeIconsNew[key],
      translations,
      korean: splitTrailingAnnotation(translations.ko_kr),
      chuNom: splitTrailingAnnotation(translations.vi_vn),
    }
  }),
} as const
